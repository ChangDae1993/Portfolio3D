...
public class CameraCtrl : MonoBehaviour
{
    ...
    private float hSpeed = 5.0f;        //마우스 좌우 회전에 대한 카메라 회전 스피드
    private float vSpeed = 2.4f;        //마우스 상하 회전에 대한 카메라 회전 스피드
    private float vMinLimit = -7.0f;    //위 아래 각도 제한
    private float vMaxLimit = 80.0f;    //위 아래 각도 제한
    private float zoomSpeed = 1.0f;     //마우스 휠 조작에 대한 줌인아웃 스피드 설정
    private float maxDist = 50.0f;      //마우스 줌 아웃 최대 거리 제한 값
    private float minDist = 3.0f;       //마우스 줌인 최소 거리 제한값
    //---카메라 위치 계산용 변수

    //---주인공을 기준으로 한 상대적인 구 좌표계 기준의 초기값
    ...
    private void StartFunc()
    {
        ...
        //---카메라 위치 계산 공식(구 좌표계를 직각 좌표계로 환산하는 부분)
        ...
        transform.position = a_BuffPos; //<-카메라 직각좌표계 기준의 위치
